...
CashoutCore.Functions.CreateCallback('cash-antihack:server:GetPermissions', function(source, cb)
...
RegisterServerEvent('cash-antihack:server:banPlayer')
AddEventHandler('cash-antihack:server:banPlayer', function(reason)
    local src = source
    TriggerClientEvent('chatMessage', -1, "q-coderc-rp Anti-Hack", "error", GetPlayerName(src).." is banned for: " ..reason )
    CashoutCore.Functions.ExecuteSql(false, "INSERT INTO `bans` (`name`, `steam`, `license`, `discord`,`ip`, `reason`, `expire`, `bannedby`) VALUES ('"..GetPlayerName(src).."', '"..GetPlayerIdentifiers(src)[1].."', '"..GetPlayerIdentifiers(src)[2].."', '"..GetPlayerIdentifiers(src)[3].."', '"..GetPlayerIdentifiers(src)[4].."', '"..reason.."', 2145913200, '"..GetPlayerName(src).."')")
    DropPlayer(src, "You have been flagged too many times by our anticheat and prevented from being banned from the server. Check our discord for more information: https://discord.gg/RMybeEU")
end)

-- Fake events --
function NonRegisteredEventCalled(CalledEvent, source)
    TriggerClientEvent("cash-antihack:client:NonRegisteredEventCalled", source, "Cheating", CalledEvent)
end
...
CashoutCore.Functions.CreateCallback('cash-antihack:server:HasWeaponInInventory', function(source, cb, WeaponInfo)
    local src = source
