    ...
    while true do
        ...
        if isLoggedIn then
            if PlayerJob.name ~= nil then
                ...
                for k, v in pairs(CashoutCore.Shared.Jobs[PlayerJob.name]["boss"]) do
...
AddEventHandler('cash_bossmenu:client:refreshSociety', function(job, data)
    if CashoutCore and CashoutCore.Functions.GetPlayerData().job.name == job then
        SendNUIMessage({
            open = true,
...
RegisterNUICallback('openStash', function(data)
    ...
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "boss_" .. CashoutCore.Functions.GetPlayerData().job.name, {
        maxweight = 4000000,
        slots = 500,
    })

    TriggerEvent("inventory:client:SetCurrentStash", "boss_" .. CashoutCore.Functions.GetPlayerData().job.name)
end)
