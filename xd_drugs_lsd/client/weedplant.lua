	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				CashoutCore.Functions.Draw2DText(0.5, 0.88, 'Press ~g~[E]~w~ to pickup acid', 0.5)
			end
			local hasBagd = false
			local s1d = false
...
--prop_barrel_01a  prop_plant_01a
		CashoutCore.Functions.SpawnLocalObject('prop_barrel_01a', weedCoords, function(obj) --- change this prop to whatever plant you are trying to use 
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				...
				elseif (hasBag) then
					CashoutCore.Functions.Notify('You dont have enough Bottle acid.', 'error')
				else
					CashoutCore.Functions.Notify('You dont have enough Bottle acid or plastic bags.', 'error')
				end
			end
		else
			Citizen.Wait(500)
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer.coords, true) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing2 then
				...
				elseif (hasWeed2) then
					CashoutCore.Functions.Notify('You dont have lsd.', 'error')
				elseif (hasBag2) then
					CashoutCore.Functions.Notify('You dont have lsd.', 'error')
				else
					CashoutCore.Functions.Notify('You dont have elsd.', 'error')
				end
			end
		else
			Citizen.Wait(500)
