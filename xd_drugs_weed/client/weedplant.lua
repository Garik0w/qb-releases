	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				CashoutCore.Functions.Draw2DText(0.5, 0.88, 'Press ~g~[E]~w~ to pickup Cannabis', 0.5)
			end
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				CashoutCore.Functions.Progressbar("search_register", "Picking up Cannabis..", 7500, false, true, {
...
function SpawnWeedPlants() --This spawns in the Weed plants, 
	while spawnedWeed < 25 do
		...
		CashoutCore.Functions.SpawnLocalObject('prop_weed_02', weedCoords, function(obj) --- change this prop to whatever plant you are trying to use 
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				...
				elseif (hasBag) then
					CashoutCore.Functions.Notify('You dont have enough cannabis.', 'error')
				else
					CashoutCore.Functions.Notify('You dont have enough cannabis or plastic bags.', 'error')
				end
			end
		else
			Citizen.Wait(500)
