...
AddEventHandler('smerfikcraft:zlomiarzzbier2', function()
    ...
    if Item == nil then
        ...
        TriggerClientEvent("inventory:client:ItemBox", _source, CashoutCore.Shared.Items['milk'], "add")
        
        TriggerClientEvent('smerfik:odmrozcrft222', _source)
        TriggerClientEvent("CashoutCore:Notify", _source, 'Collected'.. ilosc .. ' milk', "success", 7000)
   else
        ...
        TriggerClientEvent("CashoutCore:Notify", _source, 'You dont have room for more milk!', "error", 7000)
...
AddEventHandler('smerfikcraft:skup22', function()
    ...
    if Item == nil then
        TriggerClientEvent('CashoutCore:Notify', source, 'No steel', "error")  
    else
       ...
       -- local price =  math.random(50, 200) 
        ...
        TriggerClientEvent("inventory:client:ItemBox", source, CashoutCore.Shared.Items['milk'], "remove")
        
        TriggerClientEvent('sprzedawanie:jablekanim22', _source)
       -- Citizen.Wait(2000)
       -- xPlayer.Functions.AddMoney("cash", price, "sold-pawn-items")
       -- TriggerClientEvent('CashoutCore:Notify', _source, 'You sold', "success")  
       xPlayer.Functions.AddItem('milkbotle', 1)
	   TriggerClientEvent("inventory:client:ItemBox", source, CashoutCore.Shared.Items['milkbotle'], "add")
        TriggerClientEvent('odlacz:propa2', _source)
       -- TriggerClientEvent('CashoutCore:Notify', _source, 'Milked cow of buckets:' .. price .. '$', "success")  
        TriggerClientEvent('tp:misc-jobs:unlockControls', _source)
  
          --  end
        else
            TriggerClientEvent('CashoutCore:Notify', _source, 'You dont have milk', "success")  
            TriggerClientEvent('tp:misc-jobs:unlockControls', _source)
        end
     end
end)
...
AddEventHandler('milk:sell', function()
	...
	if Item == nil then
       TriggerClientEvent('CashoutCore:Notify', source, 'milkbotle', "error")  
	else
		...
		if Item.amount > 0 then
			...
			TriggerClientEvent("inventory:client:ItemBox", source, CashoutCore.Shared.Items['milkbotle'], "remove")
