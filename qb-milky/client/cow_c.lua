...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(10)
        ...
        if (GetDistanceBetweenCoords(coords, 2441.460, 4755.190, 33.70, true) < 30.0)  then
            --CashoutCore.Functions.Draw2DText({2441.06, 4755.05, 33.85 }, '~y~[E] ~w~Milk the cow', 0.5)
            DrawText3D2(2441.06, 4755.05, 33.85, "~y~[E] ~w~Milk the cow")
            --CashoutCore.Functions.Draw2DText({2443.96, 4764.95, 33.85 }, '~y~[E] ~w~Milk the cow', 0.5)
            DrawText3D2(2443.96, 4764.95, 33.85, "~y~[E] ~w~Milk the cow")
           -- CashoutCore.Functions.Draw2DText({ 2434.870, 4764.150, 33.80 }, '~y~[E] ~w~Milk the cow', 0.5)
           DrawText3D2(2434.870, 4764.150, 33.80, "~y~[E] ~w~Milk the cow")
          --  CashoutCore.Functions.Draw2DText({ 2430.76, 4773.95, 33.85 }, '~y~[E] ~w~Milk the cow', 0.5)
...
AddEventHandler('wiadro:postaw', function()
    ...
    if maitem == false then
        CashoutCore.Functions.SpawnObject(model, {
            x = x,
            y = y,
            z = z
        }, function(obj)
		...
		local s1d = false
			if(GetDistanceBetweenCoords(coords, 2500.960, 4800.750, 34.740, true) < 3.0) then
                ...
                if (hasBagd) then
                    ...
                    CashoutCore.Functions.Notify('You have no to milk .', 'error')
                end
                end
            end
        end
...
Citizen.CreateThread(function()
	...
	DrawText3D2(sellX, sellY, sellZ+0.1, "[E] Sell Milk bot")
		...
		CashoutCore.Functions.Notify('You have no more milk bottle .', 'error')
		end
		end	
	end
...
AddEventHandler('tp:milking', function(source)
    ...
    --end)
    CashoutCore.Functions.Progressbar("search_register", "processing..", 2000, false, true, {disableMovement = true,
					disableCarMovement = true,
					disableMouse = false,
					disableCombat = true,
					disableInventory = true,
